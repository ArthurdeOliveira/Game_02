...
public class World {
			...
			for(int xx = 0; xx < map.getWidth(); xx++){
				for(int yy = 0; yy < map.getHeight(); yy++){
					...
					if(pixelAtual == 0xFF000000){
						...
						Enemy enemy = new Enemy(xx*16,yy*16,16,16,0,Entity.ENEMY_RED);
						Game.entities.add(enemy);
						
					}else if(pixelAtual == 0xff00ffff) {
						Enemy enemy = new Enemy(xx*16,yy*16,16,16,0,Entity.ENEMY_BLUE);
						Game.entities.add(enemy);
	...
	public static void restartGame(){
		...
		Game.frutasAtual = 0;
		Game.frutasContagem = 0;
	
		Game.world = new World("/level"+level+".png");
